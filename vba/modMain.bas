...
Public Sub ReplaceInputs(Optional ByVal p As String = "")
    ...
    ' Layout contract: the importer reads columns by POSITION, so the input
    ' sheet's header row (row 7) must match the spec exactly, column for
    ' column. Files from any older layout would scramble columns if read by
    ' position, so every mismatch is refused with a plain answer.
    Dim expHdr() As String, ci As Long, badCol As Long
    modSpec.LoadInputCols expHdr
    badCol = 0
    For ci = 1 To modSpec.IN_NCOLS
        If Trim$(CStr(src.Cells(7, 1 + ci).Value)) <> Trim$(expHdr(ci)) Then
            badCol = ci
            Exit For
        End If
    Next ci
    If badCol > 0 Then
        ...
        Dim hint As String
        If Trim$(CStr(src.Range("B6").Value)) = "Company" _
           Or Trim$(CStr(src.Range("B7").Value)) = "Company" Then
            hint = "It was generated by an OLDER version of the app " & _
                   "(column " & badCol & " should be '" & _
                   Replace(expHdr(badCol), Chr(10), " ") & "')."
        Else
