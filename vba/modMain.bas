...
Public Sub ReplaceInputs(Optional ByVal p As String = "")
    ...
    ' must be the current app's layout (headers on row 7 with "Company" in
    ' B7 and "Investment Date" in E7). Files from older versions of the app
    ' place or name columns differently - importing those by position would
    ' scramble every column, so refuse with a plain answer instead.
    If Trim$(CStr(src.Range("B7").Value)) <> "Company" _
       Or Trim$(CStr(src.Range("E7").Value)) <> "Investment Date" Then
        ...
        If Trim$(CStr(src.Range("B6").Value)) = "Company" _
           Or Trim$(CStr(src.Range("B7").Value)) = "Company" Then
            hint = "It was generated by an OLDER version of the app " & _
                   "(its columns sit in the old positions)."
        Else
